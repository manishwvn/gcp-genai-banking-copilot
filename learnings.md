...
6. **gemini-embedding-001 does not auto-normalize truncated embeddings.** Google's documentation states that when requesting `output_dimensionality` below the model's default (3072 — we use 768), manual L2 normalization is expected; this differs from the newer `gemini-embedding-2` model, which normalizes automatically. Initially missed in this project — `embed_text()` returned raw API values unmodified. Verified against real stored Firestore data: vector magnitudes were ~0.5830–0.5845, not the expected ~1.0 unit norm. Fixed by adding an explicit L2 normalization step (divide each vector element by the vector's magnitude) after the API call. **Why it mattered:** Cosine distance is mathematically scale-invariant for a given pair of vectors, so ranking wasn't necessarily wrong in this specific case (both original query and stored vectors happened to share similar magnitude ~0.58 each) — but relying on that coincidence rather than guaranteeing unit-norm vectors was a real correctness gap, not just a documentation nitpick. **Lesson:** When an API's default behavior changes based on a parameter (here, `output_dimensionality`), always check whether documented side effects (like auto-normalization) still apply — don't assume a parameter change is "just smaller numbers, same behavior otherwise." Re-normalized vectors confirmed at 0.9999–1.0 magnitude.

### How Embeddings Actually Work (Under the Hood)

Understanding embeddings means understanding the pipeline that transforms raw text into a fixed-length numeric vector. Here's the full mechanism:

**Tokenization:** Before any model sees the text, it breaks it into tokens — typically word-fragments, sometimes full words (e.g., "embedding" might be a single token, but "embeddings" might not be; punctuation often gets its own token). This is a universal first step for any language model, not embedding-specific. For gemini-embedding-001, the tokenizer is the same as used by Gemini itself.

**Transformer with bidirectional attention:** gemini-embedding-001 is initialized from the Gemini LLM itself, then fine-tuned specifically for embeddings. The core mechanism is a Transformer — a neural network architecture built on attention. Crucially, embeddings use **bidirectional attention**, meaning each token's representation gets updated by looking at every other token in the passage simultaneously (both directions). This is why "risk is low" and "risk is not low" produce different embeddings even though they share the words "risk" and "low" — context from both sides of each token matters. (By contrast, older LLMs used left-to-right attention only, making them less context-aware.)

**Pooling:** The Transformer produces one output vector per token. A chunk with 100 tokens yields 100 vectors. A pooling step (commonly mean-pooling, though the exact learned method varies by model) combines all token vectors into a single passage-level vector. Intuitively, this is extracting "what's the overall meaning of this entire chunk" from "what's the meaning of each individual word."

**Linear projection:** After pooling, a learned linear transformation (E = W·P + b, where P is the pooled vector and W, b are learned weights) maps to the final requested output dimension. In this project, we request 768 dimensions via `output_dimensionality=768`. The model doesn't need to be re-trained for each dimension — the projection layer is flexible enough to produce any supported dimension from the same internal representation. (This is enabled by Matryoshka Representation Learning, discussed below.)

**Training via contrastive learning:** How does the model learn that "credit risk" and "loan default risk" should end up near each other in vector space, even though the surface words differ? Not via hand-labeling — via contrastive learning. The model is trained on massive datasets of (query, correct-target) positive pairs plus negative examples (deliberately hard negatives: documents that are semantically similar to the query but not actually correct answers). The training loss pulls positive pairs' embeddings together and pushes negatives apart. After millions of such examples, the model learns semantic similarity without ever seeing a labeled ontology. This is emergent knowledge.

**Matryoshka Representation Learning (MRL):** During the contrastive training just described, loss is computed not just on the full 3072-dimensional vector, but simultaneously on truncated prefixes (e.g., 768, 1536, and 3072 dimensions). This forces the model to pack the most important semantic signal into the earliest dimensions. **This is why truncating to 768 dims loses almost no quality** (~0.26% per MTEB benchmarks) — the first 768 dimensions were explicitly trained to be meaningful on their own, not just chopped off after the fact. It's deliberate.

**ELI5 tie-together:** Reading a passage is like reading a whole book chapter carefully (Transformer). Summarizing that chapter into one sentence is like pooling (collapsing all the words into one idea). Translating that summary into a fixed-length code is like projection (the 768 magic numbers). The model got good at this "summarizing and encoding" by practicing millions of times on real examples, learning which summaries should sound similar and which shouldn't (contrastive training). MRL ensures the code is compact — the most important concepts are in the early digits, not scattered evenly.

### Vector Fundamentals Beyond This Project

This project uses dense, learned vectors stored in Firestore. But the embedding field is bigger. These concepts are worth knowing:

**Dense vs. sparse vectors:** Dense vectors (what we use) have a value in every dimension, with meaning distributed across all of them — produced by neural models like gemini-embedding-001. Sparse vectors are mostly zeros, with one dimension per vocabulary word (e.g., classic BM25 keyword-weighting). Sparse is great at exact-term matching and interpretability ("dimension 42 is 'loan'"), but misses paraphrases and synonyms entirely. Dense catches semantic similarity, but black-box — you can't read what "dimension 453" means.

**Hybrid search:** In production RAG systems, combining dense (semantic) + sparse (keyword) retrieval and merging results is common — capture both "the model knows this concept" and "the exact term appears in the document." Not implemented in this project (pure dense-only). **Named limitation:** our approach could miss a chunk containing an exact regulation number or defined term that doesn't stand out semantically. Relevant in banking/compliance contexts where terminology is legally precise.

**Exact KNN vs. Approximate Nearest Neighbor (ANN):** Firestore's vector index uses exact search — compares the query vector against every indexed vector and returns the true top-k. Accurate, but scales poorly at millions of vectors. Production vector databases (Pinecone, Weaviate, Milvus, Qdrant) typically use approximate algorithms, most commonly HNSW (Hierarchical Navigable Small World) graphs — trade a tiny accuracy loss (wrong order of top-10, or rarely missing one) for massive speed gains. We don't need this at our scale (handful of chunks), but it's the reason "Firestore is for filings, not for million-scale embeddings."

**Reranking / two-stage retrieval:** Common production pattern — cheaply retrieve a broad candidate set (e.g., top 20 via fast ANN search), then run a slower/more precise reranking model over just those to pick the final top 4-5 sent to generation. This trades latency for precision, useful when "any semantic match" from ANN isn't good enough. Not implemented here (single-stage top_k=4 directly). Natural upgrade path if retrieval quality drops.

**Embedding model incompatibility:** Vectors from different embedding models (or even different versions, e.g. gemini-embedding-001 vs. gemini-embedding-2, or different Cohere models) live in **incompatible vector spaces** — cannot be compared or mixed. Upgrading embedding models requires re-embedding all existing stored data from scratch. Real operational cost worth knowing before ever changing embedding models on a live system — something to plan for, not a surprise.
